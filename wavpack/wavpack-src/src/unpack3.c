...
static void unpack_init3 (WavpackStream3 *wps)
{
    ...
    if (flags & EXTREME_DECORR) {
        for (dpp = wps->decorr_passes, ti = 0; ti < (int)sizeof (extreme_terms); ti++)
        ...
        for (dpp = wps->decorr_passes, ti = 0; ti < (int)sizeof (default_terms); ti++)
            if (default_terms [sizeof (default_terms) - ti - 1] > 0 || (flags & CROSS_DECORR))
                dpp++->term = default_terms [sizeof (default_terms) - ti - 1];
    }
    else
        for (dpp = wps->decorr_passes, ti = 0; ti < (int)sizeof (simple_terms); ti++)
        ...
        if (flags & FAST_FLAG) {
            ...
            for (bptr = buffer, i = 0; i < sample_count; ++i) {
                ...
                if (flags & CROSS_DECORR) {
                    ...
                    sum_level = (int)(sum_level - (sum_level >> 8) + labs (sum >> 1));
                    left_level = (int)(left_level - (left_level >> 8) + labs (left));
                    right_level = (int)(right_level - (right_level >> 8) + labs (right));
                    diff_level = (int)(diff_level - (diff_level >> 8) + labs (diff));
...
static int32_t FASTCALL get_word2 (WavpackStream3 *wps, int chan)
{
    ...
    if (wps->wphdr.bits) {
        for (value = (int)(1L << (dbits - 1)); --dbits; mask <<= 1)
...
static int32_t FASTCALL get_word4 (WavpackStream3 *wps, int chan, int32_t *correction)
{
    ...
    wps->w4.fast_level [chan] += (avalue = (int)labs (mid));
    ...
    if (bs_is_open (&wps->wvcbits)) {
        ...
        if (high != low) {
            ...
            uint32_t extras = (uint)((1L << bitcount) - maxcode - 1);
