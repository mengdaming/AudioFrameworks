...
public:
  ...
  TagPrivate(File *p_document) :
    ...
    albumArtist(document->d->tags.end()),
    ...
    track(document->d->tags.end())
  {
    ...
    for(List<std::pair<PropertyMap, std::pair<Element *, ulli> > >::Iterator i =
      ...
      // Same goes for album artist.
      if(i->first.find(Constants::ALBUM_ARTIST) != i->first.end() &&
        (albumArtist == document->d->tags.end() ||
        albumArtist->second.second > i->second.second ||
        i->second.second == Constants::MostCommonPartValue)) {
  ...
  List<std::pair<PropertyMap, std::pair<Element *, ulli> > >::Iterator albumArtist;
...
String EBML::Matroska::File::Tag::albumArtist() const
{
  if(e->albumArtist != e->document->d->tags.end())
    return e->albumArtist->first.find(Constants::ALBUM_ARTIST)->second.front();
  else
    return String();
}
...
void EBML::Matroska::File::Tag::setAlbumArtist(const String &s)
{
  if(e->albumArtist != e->document->d->tags.end())
    e->update(e->albumArtist, Constants::ALBUM_ARTIST, s);
  else
    e->insert(Constants::ALBUM_ARTIST, Constants::MostCommonPartValue, s);
}
