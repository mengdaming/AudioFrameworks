...
int CAPEDecompressOld::Seek(intn nBlockOffset)
{
	...
	spTempBuffer.Assign(new char[nMaximumDecompressedFrameBytes + 16], true);
    ZeroMemory(spTempBuffer.GetPtr(), nMaximumDecompressedFrameBytes + 16);
    ...
    memcpy(&m_spBuffer[m_nBufferTail], &spTempBuffer[nBytesToSkip], nBytesToKeep);
